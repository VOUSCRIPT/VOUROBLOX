...
local function antiAntiSpy()
    
    coroutine.wrap(function()
        while wait(math.random(5,15)) do
            pcall(function()
                SimpleSpy2.Name = "ScreenGui_"..tostring(math.random(10000,99999))
                Background.Name = "Frame_"..tostring(math.random(100,999))
            end)
        end
    end)()
    ...
    for i=1, math.random(5,10) do
        dummyData[i] = string.rept(tostring(math.random()), 10000)
    end
end
...
function v2s(v, l, p, n, vtv, i, pt, path, tables, tI)
    -- 添加对更多数据类型的支持
    if typeof(v) == "userdata" and not rawequal(v, nil) then
        return "newproxy(true) --[[HIDDEN USERDATA]]"
    end
    if not tI then
        tI = { 0 }
	else
...
function remoteHandler(hookfunction, methodName, remote, args, funcInfo, calling, returnValue)
    ...
    if randomVal > 95 then  -- 5%几率添加随机延迟
        wait(math.random(10,50)/1000)
    end
    
    -- 增强远程检测能力
    local validInstance, validClass = pcall(function()
        return remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction") or 
               (typeof(remote) == "userdata" and tostring(remote):find("Remote"))
    end)
    
    

	if validInstance and validClass then -- 添加随机延迟和伪装
	 local randomVal = math.random(1,100)
	  if randomVal > 95 then -- 5%几率添加随机延迟 
	 wait(math.random(10,50)/1000) end
...
function toggleSpy()
    if not toggle then
        ...
        if hookmetamethod then
            -- 伪装成普通函数调用
            local fakeName = string.char(math.random(65,90))..string.char(math.random(65,90)).."_"..tostring(math.random(100,999))
            _G[fakeName] = function(...) return newnamecall(...) end
           
            
              
            local oldNamecall = hookmetamethod(game, "__namecall", _G[fakeName])
