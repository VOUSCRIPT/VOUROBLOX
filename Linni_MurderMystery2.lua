...
Converted["_Linni"].DisplayOrder = 3
Converted["_Linni"].IgnoreGuiInset = true
Converted["_Linni"].ScreenInsets = Enum.ScreenInsets.DeviceSafeInsets
Converted["_Linni"].ResetOnSpawn = false
Converted["_Linni"].ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Converted["_Linni"].Name = "Linni"
Converted["_Linni"].Parent = game:GetService("CoreGui")

Converted["_Open"].Font = Enum.Font.Gotham
Converted["_Open"].Text = "三击此区域打开霖溺"
...
Converted["_Open"].Parent = Converted["_Linni"]
...
Converted["_ListButton"].Parent = Converted["_Linni"]
...
Converted["_Notifications"].Parent = Converted["_Linni"]
...
Converted["_TextBoxPlaceholder"].Parent = Converted["_Linni"]
...
Converted["_FloatingButton"].Text = "射杀杀手"
...
Converted["_FloatingButton"].Parent = Converted["_Linni"]
...
Converted["_FloatingButtons"].Parent = Converted["_Linni"]
...
Converted["_AddCustomModule"].Parent = Converted["_Linni"]
...
Converted["_DroppedGunBGUI"].Parent = Converted["_Linni"]

Converted["_TextLabel3"].Font = Enum.Font.SourceSansBold
Converted["_TextLabel3"].Text = "掉落的枪"
...
Converted["_MurdererBGUI"].Parent = Converted["_Linni"]

Converted["_TextLabel4"].Font = Enum.Font.SourceSansBold
Converted["_TextLabel4"].Text = "杀手"
...
Converted["_Menu"].Parent = Converted["_Linni"]
...
Converted["_AddCustomModule1"].Text = "霖溺_Linni"
...
Converted["_HubCredits"].Text = "由霖溺制作"
...
Converted["_HubDesc"].Text = "随机中心的菜单"
...
Converted["_TextLabel5"].Text = "欢迎使用脚本"
...
Converted["_TextLabel6"].Text = "防止说动蛋糕部分功能为英文"
...
Converted["_TextLabel7"].Text = "Linni"
...
Converted["_TextLabel8"].Text = "点击此处最小化"
...
Converted["_Toggle"].Parent = Converted["_Linni"]

Converted["_TextLabel9"].Font = Enum.Font.Unknown
Converted["_TextLabel9"].Text = "循环调整速度和视野"
...
Converted["_Modules"].Parent = Converted["_Linni"]
...
Converted["_DropdownFrameSample"].Parent = Converted["_Linni"]
...
Converted["_Sample"].Text = "这里可以放很多文字."
...
Converted["_Range"].Parent = Converted["_Linni"]
...
Converted["_Dialog"].Parent = Converted["_Linni"]
...
Converted["_NotificationSample"].Parent = Converted["_Linni"]
...
do
    ...
    script.Parent = Converted["_Linni"]
    local function module_script()
		...
		function FUNCTIONSmodule.createFloatingButton(item,button,buttonname)
			if not _G.Linni.FloatingButtons:FindFirstChild(string.gsub(buttonname, "_", "")) then
		
		
				local newFloatingButton = _G.Linni.FloatingButton:Clone()
				newFloatingButton.Parent = _G.Linni.FloatingButtons
				...
				task.spawn(function()
					local buttontodestroy = _G.Linni.FloatingButtons:FindFirstChild(string.gsub(buttonname, "_", ""))
		...
		function loader(module)
			...
			for _, item in ipairs(module) do
				...
				elseif item["Type"] == "Input" then
					local cloneinput = _G.Linni.TextBoxPlaceholder:Clone()
				...
				elseif item["Type"] == "Toggle" then
					local clonetoggle = _G.Linni.Toggle:Clone()
				...
				elseif item["Type"] == "Dropdown" then	
					local clonedropdown = _G.Linni.Dropdown:Clone()
					local dropdownFrame = _G.Linni.DropdownFrameSample
		...
		function FUNCTIONSmodule.refreshlist()
			...
			for modulei, module in ipairs(_G.Modules) do
				local success, err = pcall(function()
					local listbutton = _G.Linni.ListButton:Clone()
				...
				if not success then
					warn("[Linni] Error while requiring " .. module.Name .. "!")
					warn(err)
				end
			end
		end
		...
		_G.LinniFUNCTIONS = FUNCTIONSmodule
...
local function JQJVD_fake_script()
...
local function QFRK_fake_script()
    local script = Instance.new("LocalScript")
    script.Name = "OnClick"
    script.Parent = Converted["_Open"]
    local req = require
	...
	script.Parent.MouseButton1Click:Connect(function()
		...
		if clickCount == 3 then
			ts:Create(_G.Linni.Menu, TweenInfo.new(0.7, Enum.EasingStyle.Back, Enum.EasingDirection.Out), 
...
local function KBEFORH_fake_script()
    local script = Instance.new("LocalScript")
    script.Name = "Resizer"
    script.Parent = Converted["_Open"]
    local req = require
...
local function VXLACE_fake_script()
    local script = Instance.new("LocalScript")
    script.Name = "Init"
    script.Parent = Converted["_Linni"]
    local req = require
	...
	local s, e = pcall(function()
		if get_hidden_gui or gethui then
			...
			print("[Linni] - Using get_hidden_gui for anti-detection.")
		elseif (not is_sirhurt_closure) and (syn and syn.protect_gui) then
			script.Parent.Name = randomString()
			syn.protect_gui(script.Parent)
			script.Parent.Parent = COREGUI
			print("[Linni] - Using syn.protect_gui for anti-detection.")
		elseif COREGUI:FindFirstChild('RobloxGui') then
			script.Parent.Parent = COREGUI.RobloxGui
			print("[Linni] - Using RobloxGui for anti-detection.")
		else
			warn("[Linni] - Using CoreGui as anti-detection. This is the most basic coverage and can still be detected.")
		end
	end)
	
	print("[Linni] - Linni is now in " .. tostring(script.Parent:GetFullName()))
	if not s then
		warn("[Linni] - Attempts at anti-detection failed. Using CoreGui as anti-detection.")
		warn(e)	
	end
	
	printidentity("[Linni] - Your executor level (identity) is")
	
	local getExeName = identifyexecutor or getexecutorname or function() return "Yet Another Roblox Executor v1.2" end
	print("[Linni] - Your executor is " .. getExeName())
	...
	require(script.Parent.FUNCTIONS).notification("Welcome to Linni Astralfire.")
...
local function JGXWB_fake_script() 
    local script = Instance.new("LocalScript")
...
local function MGTT_fake_script() 
    local script = Instance.new("LocalScript")
...
local function TKTMNY_fake_script()
    local script = Instance.new("LocalScript")
...
local function ZNJPL_fake_script() 
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
...
local function FHXBSSY_fake_script()
    local script = Instance.new("LocalScript")
...
local function PCGAKG_fake_script()
    local script = Instance.new("LocalScript")
...
local function HXNHH_fake_script() 
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
...
local function DGSNCYE_fake_script()
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
	...
	script.Parent.MouseButton1Click:Connect(function()
		ts:Create(_G.Linni.Menu, TweenInfo.new(0.4, Enum.EasingStyle.Circular, Enum.EasingDirection.Out), 
...
local function BVLUONR_fake_script() 
    local script = Instance.new("LocalScript")
...
local function TDXDSDS_fake_script()
    local script = Instance.new("LocalScript")
    script.Name = "《逃离设施》"
    script.Parent = Converted["_Linni"]
    local req = require
	...
	module[6] = {
		Type = "Input",
		Args = {"输入移动速度", "设置并锁定", function(Self, text)
			if not tonumber(text) then
				FUNCTIONS.notification("输入的数字无效")
				return
			end
...
local function VHCKUS_fake_script() 
    local script = Instance.new("LocalScript")
    script.Name = "霖溺-破坏者谜团2"
    script.Parent = Converted["_Linni"]
    local req = require
    local require = function(obj)
        local fake = fake_module_scripts[obj]
	...
	local fu = require(_G.Linni.FUNCTIONS)
	if not game.ReplicatedStorage:WaitForChild("Remotes", 10) then
		fu.dialog("不是破坏者谜团2", "看起来这不是《破坏者谜团2》游戏。你仍要加载模块吗？", {"加载", "不"})
	
		if fu.waitfordialog() == "No" then
			fu.closedialog()
			fu.notification("在你重新加入前，不会加载破坏者谜团2.", Color3.fromRGB(255, 0, 0), "x")
			return
		end	
		fu.closedialog()
	else
	...
	module["Name"] = "霖溺-破坏者谜团2"
	
	workspace.ChildAdded:Connect(function(ch)
		if ch == getMap() and playerESP then
			fu.notification("地图已加载等待分配角色...")
			repeat
				task.wait(1)
			until findMurderer()
			
			fu.notification("玩家ESP已重新加载.")
		end
	end)
	
	workspace.ChildRemoved:Connect(function(ch)
		if ch == getMap() and playerESP then
			fu.notification("游戏结束移除玩家ESP")
			playerData = {}
			if _G.Linni:FindFirstChild("AppliedMurdererBGUI") then _G.Linni:FindFirstChild("AppliedMurdererBGUI"):Destroy() end
			if _G.Linni:FindFirstChild("DGBGUIClone") then _G.Linni:FindFirstChild("DGBGUIClone"):Destroy() end
			for _, v in ipairs(script.Parent:GetChildren()) do if v.Name == "PlayerESP" then v:Destroy() end end
		end
	end)
	...
	workspace.DescendantAdded:Connect(function(ch)
		...
		if gunDropESP and ch.Name == "GunDrop" then
			...
			fu.notification("枪已掉落！寻找黄色高亮。.")
			if autoGetDroppedGun then
				fu.notification("自动拾取掉落枪支 - 冷却中...")
				task.wait(1)
				if not getMap():FindFirstChild("GunDrop") then fu.notification("没有可传送到的掉落枪支") return end
	...
	workspace.DescendantRemoving:Connect(function(ch)
		if gunDropESP and ch.Name == "GunDrop" then
			...
			task.wait(0.6)
			fu.notification("英雄是 " .. findSheriff().DisplayName .. ".")
			if playerESP then
	...
	table.insert(module, {
		...
		Args = {2, {
			...
			Dropped_Gun = function()
				if gunDropESP then
					gunDropESP = false
					if _G.Linni:FindFirstChild("GunESP") then _G.Linni:FindFirstChild("GunESP"):Destroy() end
					if _G.Linni:FindFirstChild("DGBGUIClone") then _G.Linni:FindFirstChild("DGBGUIClone"):Destroy() end
				else
					gunDropESP = true
			...
			Auto_Shoot_murderer = function()
				...
				if findSheriff() == localplayer and autoShooting then
					...
					repeat
						task.wait(0.1)
						local murderer = findMurderer() or findSheriffThatsNotMe()
						if not murderer then warn("[Linni] > MM2 Autoshoot - No murderer.") continue end
						local murdererPosition = murderer.Character.HumanoidRootPart.Position
						local characterRootPart = localplayer.Character.HumanoidRootPart
	...
	table.insert(module, {
		Type = "Button",
		Args = {"在聊天中发送警长和凶手的名字", function(Self)
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"放下时自动取枪", function(Self, state)
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"忽略飞刀（不起作用）", function(Self, state)
	...
	table.insert(module, {
		Type = "Button",
		Args = {"上帝模式（非常非常不稳定）", function(Self) 
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"模拟飞刀击杀最近距离", function(Self, state)
			simulateKnifeThrow = state
			if state then
				fu.notification("模拟飞刀可以让你看起来合法。但是，请注意，它的可靠性较低，可能会错过目标。.")
			end
		end,}
	})
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"凶手杀戮光环", function(Self, state)
			if state then
	...
	table.insert(module, {
		Type = "Button",
		Args = {"把所有人都扣为人质", function()
			if findMurderer() ~= localplayer then fu.notification("你不是凶手。这只有在你是凶手的情况下才有用.") return end
			...
			fu.notification(".把每个球员都放在一个点上。一旦你决定，立刻杀死所有人")
		end,}
	})
...
local function COBWDKS_fake_script() 
    local script = Instance.new("LocalScript")
    script.Name = "主要功能"
    script.Parent = Converted["_Linni"]
    local req = require
	...
	table.insert(module, {
		...
		Args = {"AFEM - A free, FE, simple and easy emotes menu!", function()
			loadstring(game:HttpGet("https://Linni.mhi.im/scr?channel=afem", false))()
		end,}
	})
	...
	table.insert(module, {
		Type = "Button",
		Args = {"加入霖溺QQ群聊", function(Self)
			if setclipboard then setclipboard("霖溺(感谢老铁支持)QQ新主群：http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=viOjjgj19-TUiZlamhpxb6uvWwVNGoB7&authKey=ACDi9sCtIis%2F4P%2BtirP046uWaJ54%2F149eBnUvaAsMu%2BWZwSFoEQrzZC9UDGFwmp%2F&noverify=0&group_code=744830231") end
			fu.notification('复制成功!')
		end,}
	})
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"循环调整速度和视野", function(Self, state)
			loopfovandws = state
		end,}
	})
					...
					if Character and Humanoid and RootPart then
						...
						if THead then
							if THead.Velocity.Magnitude > 500 then
								fu.dialog("玩家被甩飞", "玩家已被甩飞，再次甩飞？?", {"再次甩飞", "不"})
						...
						elseif not TRootPart and not THead and Accessory and Handle then
							...
							fu.notification("找不到目标玩家的合适部位进行甩飞.")
						end
						...
						fu.notification("目标玩家没有有效角色，可能已死亡.")
					end
				end
				SkidFling(Targets[1])
	...
	table.insert(module, {
		...
		Args = {"Hide Linni", function(Self)
			...
			elseif uis.AccelerometerEnabled then
				...
				fu.notification("Can't hide Linni!") 
			end
		end,}
	}
	)
	...
	table.insert(module, {
		Type = "Button",
		Args = {"FPS提升", function(Self)
			fu.dialog("FPS提升", "FPS提升可能会产生不可预测的效果。相反，使用这个你可能会落后更多!", {"无论如何，FPS提升", "别介意"})
	...
	table.insert(module, {
		...
		Args = {2, {
			...
			Reclip = function()
				...
				fu.notification(".重新剪辑可能需要你重新设置你的角色。")
			end,
		}}})
	...
	table.insert(module, {
		Type = "Button",
		Args = {"锁定/解锁浮动按钮", function()
			fu.lockMode = true
			fu.notification(".单击/点击浮动按钮以锁定/解锁..")
		end,}
	})

	table.insert(module, {
		Type = "Button",
		Args = {"获取Ping", function(Self)
	...
	table.insert(module, {
		Type = "Button",
		Args = {"打开开发人员控制台（调试）", function(Self)
	...
	local usernameColors = {
		...
		["Linniplus"] = Color3.fromHex("#f48fff"),
	...
	local mainText = "Linni Developer"
	local mainFont = Enum.Font.GothamBold
	local specialText = "Linni+"
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"隐藏Linni+/Developer标签", function(Self, state)
